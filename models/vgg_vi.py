...
class VGG_vi(nn.Module):
    def __init__(self, sigma_0, N, init_s, vgg_name='VGG16', nclass=10, img_width=32):
    ...
    def forward(self, x, fix=False, sample=True):
        kl_sum = 0
        out = x
        for l in self.features:
            if type(l).__name__.startswith("Rand"):
                out, kl = l.forward(out, fix=fix, sample=sample)
                if kl is not None:
                    kl_sum += kl
            else:
                out = l.forward(out)
        out = out.view(out.size(0), -1)
        out, kl = self.classifier.forward(out, fix=fix, sample=sample)
        kl_sum += kl
        return out, kl_sum
